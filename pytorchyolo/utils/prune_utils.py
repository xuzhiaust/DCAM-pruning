...
#步长为2的忽略，为了保持特征形状，每个res模块的最后一层不剪枝，输出层不剪枝,upsanmple的conv不剪枝
...
def obtain_bn_mask(bn_module, thre):
    ...
    thre = thre.cuda()
    #ge(),满足大于等于的为true，否则为false,.float()将bool转化为浮点数，方便后面与原始权重相乘，否则还要转换一次。
    #因为与1相乘表示保留，与0相乘表示删除，因此大于等于就相当于先筛选保留的卷积核
    mask = bn_module.weight.data.abs().ge(thre).float()

    return mask

def obtatin_attention_mask(record_sum,thre):
    record_sum_tmp=torch.from_numpy(record_sum.reshape(record_sum.shape[1]))
    mask=record_sum_tmp.ge(thre).float()
    return mask